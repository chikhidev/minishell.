...
int saver(t_db *db, t_tokenizer *self)
{
    ...
    if (db->curr_type != INVALID)
    {
        ...
        while (self->line[self->it.j] &&
                ...
                !(validate_io(&self->line[self->it.j], 1) != INVALID || validate_io(&self->line[self->it.j], 2) != INVALID))
        {
            self->it.j++;
        }
...
char **tokenize(t_db *db, t_quote **quotes, char *s)
{
    ...
    while (self.it.i < len)
    {
        bool is_open_whitespace_ = is_open_whitespace(self.line, self.it.i, *self.quotes);
        bool is_open_io_ = is_open_io(self.line, self.it.i, *self.quotes);
        bool is_quote_ = is_quote_oppening(*self.quotes, self.it.i) && (self.it.i > 0) && (self.line[self.it.i - 1] != '=')
            && (self.line[self.it.i - 1] != '$');

        if (!is_open_whitespace_ && !is_open_io_ && !is_quote_ && self.read_write_perm)
        ...
        else if (is_quote_ && self.read_write_perm && self.save == NULL)
        {
            ...
            self.save = ft_substr(db, self.line, self.it.i, self.it.j - self.it.i + 1);
            self.result = append_word(db, self.result, self.save);
            self.save = NULL;
            self.it.i = self.it.j;
        }
