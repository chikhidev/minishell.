...
/*   Updated: 2024/09/12 02:48:17 by abchikhi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		unsigned char type)
{
	...
	while (curr)
	{
		if (ft_strcmp(curr->entry->d_name, name) == 0
			&& curr->entry->d_type == type)
			return (curr);
		curr = curr->next;
	}
	return (curr);
...
void	del_ent_node(t_file_entry **list, char *name, unsigned char type)
{
	...
	if (prev)
	{
		if (ft_strcmp(name, prev->entry->d_name) == 0
			&& prev->entry->d_type == type)
		{
			*list = (*list)->next;
			return ;
		}
		...
		while (curr)
		{
			if (ft_strcmp(name, curr->entry->d_name) == 0
				&& curr->entry->d_type == type)
			{
				prev->next = curr->next;
				return ;
