...
int update_index(t_db *db, char **line, char *var, t_str_cut *cut)
{
    ...
    {
        insert = ft_substr(db, *line, cut->start_include, cut->end_include - cut->start_include + 1);
        tmp = ft_strjoin(db, left_section, insert);
    }
    right_section = ft_substr(db, *line, cut->end_ignore, ft_strlen(*line) - cut->end_ignore);
